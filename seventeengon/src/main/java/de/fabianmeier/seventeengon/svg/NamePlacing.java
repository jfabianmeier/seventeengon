...
public class NamePlacing
{
	 ...
	 */
	public static XYvector getMidVector(XYvector direction1,
			XYvector direction2, double boundingWidth, double boundingHeight)
	{
		...
		else
		{
			...
			if (quadrant1 == 1)
			{
				diagonalLine = new Line(new XYpoint(0, boundingHeight),
						new XYpoint(boundingWidth, 0), 0, 1);
			}
			if (quadrant1 == 2)
			{
				diagonalLine = new Line(new XYpoint(0, boundingHeight),
			...
			double diagonalLength = Math.sqrt(boundingWidth * boundingWidth
					+ boundingHeight * boundingHeight);

			XYpoint origin = new XYpoint(0, 0);

			Line direction1Line = new Line(origin,
					direction1.multiplyBy(diagonalLength).shift(origin), 0, 1);

			Line direction2Line = new Line(origin,
					direction2.multiplyBy(diagonalLength).shift(origin), 0, 1);

			XYpoint dirPoint1 = (XYpoint) diagonalLine
					.intersectWith(direction1Line).normalize();
			XYpoint dirPoint2 = (XYpoint) diagonalLine
					.intersectWith(direction2Line).normalize();

			double length12 = new Line(dirPoint1, dirPoint2, 0, 1).getLength();

			double factor = diagonalLength / length12;

			XYpoint midPoint = new XYpoint(
					dirPoint1.getX() / 2 + dirPoint2.getX() / 2,
	 ...
	 */
	private static XYvector getQuadrantVector(int quadrant, double xOffset,
			double yOffset)
	{
		quadrant = quadrant % 4;
		if (quadrant == 0)
